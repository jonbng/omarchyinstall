...
10. Hash the password with SHA-512 crypt (`$6$`, `openssl passwd -6` compatible) in Rust. Write cidata files onto the FAT volume (see Cidata autoinstall). **Do not persist the plaintext password in `state.json` after hashing.** Set the FAT partition no-default-drive-letter, but not hidden: Windows makes a hidden FAT32 volume unreachable through its saved volume GUID. FAT32 has no Windows ACL support, so the absent drive letter only reduces accidental exposure and is not an access-control boundary against an administrator.
...
- No-default-drive-letter and non-hidden attributes on the cidata volume; document that hiding FAT32 breaks saved-GUID access and that FAT32 cannot enforce an ACL
